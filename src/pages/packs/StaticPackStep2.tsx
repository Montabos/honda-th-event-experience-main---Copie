...
const StaticPackStep2 = () => {
  ...
  const { selectedDate, selectedDateId, packType, accompanying, totalPrice } = location.state || {};
  ...
  const [subtotal, setSubtotal] = useState(() => {
    ...
    const basePrice = packType === 'weekend' ? 30 : 20;
    // Le premier accompagnant est gratuit, on ne compte que les accompagnants supplémentaires
    const accompanyingCost = Math.max(0, accompanying - 1) * 5;
    return basePrice + accompanyingCost;
  });
  ...
  useEffect(() => {
    if (!selectedDate || !packType) {
      navigate('/pack/statique');
    }
  }, [selectedDate, packType, navigate]);
